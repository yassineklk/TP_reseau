...
static void app(void)
{
   ...
   Client disconnectedClients[MAX_CLIENTS];
   fd_set rdfs;
      ...
      else
      {
         ...
         for(i = 0; i < actual; i++)
         {
            ...
            if(FD_ISSET(clients[i].sock, &rdfs))
            {
               ...
               strncpy( content, buffer, BUF_SIZE );
               for(j = 0; j < actual; j++)
               { 
                 if (strstr(content,clients[j].name)!=NULL)
                 {
                     printf("%s  \n \r",content);
                     pv =1;
                 } 
...
static void send_message_to_all_clients(Client *clients, Client sender, int actual, const char *buffer, char from_server)
{
   ...
   for(i = 0; i < actual; i++)
   {
      ...
      if(sender.sock != clients[i].sock && clients[i].state !=0)
      {
         if(from_server == 0)
         {
